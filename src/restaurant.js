...
const calculateTotalAmount = (request) => {
  const value = request.reduce((acc, cur) => acc + cur);
  const increase = value * 0.1;
...
const createMenu = (object) => {
  const restaurant = {
    ...
    pay() {
      ...
      const menu = [Object.assign(food, drink)];
      const request = [];
      consumption.forEach((element) => request.push(menu[0][element]));
